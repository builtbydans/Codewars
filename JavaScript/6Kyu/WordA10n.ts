...
// any other character like a space or hyphen (eg. "elephant-ride") will split
// up a series of letters into two words (eg. "elephant" and "ride").
...
export function abbreviate(str: string): string {
  ...
  for (let i = 0; i < words.length; i++) {
    if (words[i].length > 3 && /[a-zA-Z]/.test(words[i])) {
      const firstL = words[i][0];
      const lastL = words[i].at(-1);
      const subset = words[i].slice(1, -1).length;
      abbreviated.push(`${firstL + subset + lastL}`);
    } else {
      abbreviated.push(words[i]);
    }
  }
...
console.log(abbreviate("hel")); // hel
