/*
    ...
    is in upper case) or (b) considered to be an exception and put entirely into 
...
const titleCase = (title, minorWords) => {
  ...
  const testWords = minorWords ? minorWords.toLowerCase().split(" ") : [];
  const split = title.toLowerCase().split(" ");
  let fixedWord = [];

  fixedWord.push(split[0][0].toUpperCase() + split[0].slice(1));

  for (let i = 1; i < split.length; i++) {
    if (!testWords.includes(split[i])) {
      fixedWord.push(split[i][0].toUpperCase() + split[i].slice(1));
    } else {
      fixedWord.push(split[i]);
    }
  }
